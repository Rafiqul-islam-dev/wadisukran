...
const restoreAgent = (id) => {
    ...
    router.get(route('agents.restore', id), {}, {
        onSuccess: () => {
...
const confirmDelete = () => {
    if (deletingUser.value) {
        router.delete(route('agents.permanent-delete', deletingUser.value.id), {
            onSuccess: () => {
                            ...
                            <tr v-for="user in users" :key="user.id"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex space-x-2">
                                        <button v-if="can('agent restore')" @click="restoreAgent(user.id)"
                                            class="flex gap-2 text-blue-600 hover:text-blue-800 font-medium transition-all duration-200 px-3 py-1 rounded-lg hover:bg-blue-50 hover:shadow-md">
                                            <Rotate3D />
                                            Restore
                                        </button>
                                        <button v-if="can('agent permanent delete')" @click="deleteUser(user)"
