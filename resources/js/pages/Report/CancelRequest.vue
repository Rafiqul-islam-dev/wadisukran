...
const handleSearch = () => {
    router.get(
        route('reports.cancel-request'),
        {
            ...filter.value,
            status: 'Cancel-Request' // Match database status value
        },
        {
            preserveScroll: true,
            replace: true,
...
const acceptCancel = (orderId: number) => {
  router.put(
    route('reports.cancel-accept', orderId),
    {},
    {
      preserveScroll: true,
      onSuccess: () => {
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-for="(order, index) in orders" :key="order.id"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <button
                                    @click="acceptCancel(order.id)"
                                    class="cursor-pointer px-4 py-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl hover:from-orange-600 hover:to-amber-600 transition-all duration-200 font-semibold shadow-lg">
                                    Accept
                                    </button>
